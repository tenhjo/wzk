...
def perlin_noise_2d(
    ...
    Raises:
        ValueError: If the shape is not a multiple of res.
    """
...
def __input_wrapper(
    n_dim: int, shape: tuple[int, ...], res: ArrayLike, tileable: tuple[bool, ...] | None, seed: int | None
) -> tuple[np.ndarray, np.ndarray, tuple[bool, ...], np.ndarray, np.ndarray]:
    if seed is not None:
        np.random.seed(seed)
...
def fractal_noise(
    ...
    Raises:
        ValueError: If the shape is not a multiple of
            (lacunarity**(octaves-1)*res).
    """
    ...
    for _ in range(octaves):
        noise += amplitude * perlin_noise_3d(
            shape=shape, res=tuple(np.array(res) * frequency), tileable=tileable, interpolant=interpolant
        )
