...
def tile_offset(a: ArrayLike, reps: int | tuple[int, ...], offsets: ArrayLike | None = None) -> jax.Array:
    ...
    if offsets is not None:
        ...
        assert len(o) == len(s)
        offsets_grid = [jnp.repeat(jnp.arange(rr), ss) * oo for ss, rr, oo in zip(s, r, o, strict=False)]
...
def tile_2d(
    *,
    ...
    ll = (v_to_next_row[0] + offset[0], v_to_next_row[1] + offset[1])

    img = img[ll[0] : ll[0] + shape[0], ll[1] : ll[1] + shape[1]]
...
def block_collage(
    *,
    ...
    img = np.full(
        shape=(n_x * n_rows + bv_i * (n_rows - 1) + 2 * bv_o, n_y * n_cols + bh_i * (n_cols - 1) + 2 * bh_o),
        fill_value=fill_boarder,
        dtype=dtype,
    )
    ...
    for r in range(n_rows):
        for c in range(n_cols):
            img[
                bv_o + r * (n_y + bv_i) : bv_o + (r + 1) * (n_y + bv_i) - bv_i,
                bh_o + c * (n_x + bh_i) : bh_o + (c + 1) * (n_x + bh_i) - bh_i,
            ] = img_arr[r, c]
