...
def sample_24cube_rotations(n: int | None = None, i: np.ndarray | None = None) -> np.ndarray:
    if i is None:
...
def xy2cube24(x: int, y: int) -> np.ndarray:
    """
    face = [+-1, +-2, +-3]
...
def get_all_24_cube_rotations() -> np.ndarray:
    all_xy = [+1, +2, +3, -1, -2, -3]
