...
from wzk.math.math2 import random_subset
...
def kofn(
    ...
    pop_size: int = 200,
    pop0: np.ndarray | None = None,
    n_gen: int = 500,
    n_keep_best: int | None = None,
    tourney_size: int | None = None,
    mut_prob: float = 0.01,
    mut_frac: float | None = None,
    mutate_best: int = 0,
    log_level: int = 0,
    ...
    for g in range(n_gen):
        ...
        if offspring.shape[0] < pop_size - n_keep_best:
            offspring = np.concatenate(
                [offspring, random_subset(n=n, k=k, m=pop_size - n_keep_best - offspring.shape[0])], axis=0
            )

        fitness = fitness_fun(offspring)

        pop, fitness = keep_best(
            pop=offspring, pop_old=pop, fitness=fitness, fitness_old=fitness_old, n_keep_best=n_keep_best
        )
...
def keep_best(
    *, pop: np.ndarray, pop_old: np.ndarray, fitness: np.ndarray, fitness_old: np.ndarray, n_keep_best: int = 0
) -> tuple[np.ndarray, np.ndarray]:
